...
function obtenerNombreGradoSecundaria($grado) {
    $grados = array(1 => '7mo', 2 => '8vo', 3 => '9no');
...
$pdf->Cell(0, 10, 'SECUNDARIA', 1, 1, 'C', true);
$pdf->Ln(3);
...
if ($result_porcentajes && mysql_num_rows($result_porcentajes) > 0) {
    while ($row_porc = mysql_fetch_assoc($result_porcentajes)) {
        ...
        while ($alumno = mysql_fetch_assoc($result_alumnos)) {
            $grado_texto = obtenerNombreGradoSecundaria($alumno['alumno_grado']);
            $pdf->Cell(10, 6, '', 0, 0);
    ...
    $pdf->Cell(0, 8, 'TOTAL SECUNDARIA: ' . $total_secundaria . ' alumnos', 1, 1, 'C', true);
} else {
    $pdf->SetFont('Arial', 'I', 11);
    $pdf->Cell(0, 8, 'No hay alumnos con beca en Secundaria', 1, 1, 'C');
}

mysql_close($conn);
$pdf->Output('I', 'Reporte_Detallado_Secundaria.pdf');
?>
