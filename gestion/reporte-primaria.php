...
$pdf->Cell(0, 9, 'PRIMARIA', 1, 1, 'C', true);
$pdf->Ln(2);

// Consulta para Primaria (nivel 3)
$query_primaria = "
    SELECT 
        ab.beca_porcentaje,
...
$result_primaria = mysql_query($query_primaria, $conn);
$total_primaria_winston = 0;

if ($result_primaria && mysql_num_rows($result_primaria) > 0) {
    ...
    while ($row = mysql_fetch_assoc($result_primaria)) {
        $pdf->Cell(95, 6, $row['beca_porcentaje'] . '%', 1, 0, 'C');
        $pdf->Cell(95, 6, $row['total_alumnos'], 1, 1, 'C');
        $total_primaria_winston += $row['total_alumnos'];
    ...
    $pdf->Cell(95, 7, $total_primaria_winston, 1, 1, 'C', true);
} else {
...
// Datos estaticos de AVANZA para Primaria
$avanza_primaria = array(
    20 => 66,
    50 => 1,
    90 => 1
);
...
$pdf->SetFont('Arial', '', 10);
$total_avanza = 0;
foreach ($avanza_primaria as $porcentaje => $cantidad) {
...
// Generar PDF
$pdf->Output('I', 'Reporte_Becas_Primaria.pdf');
?>
