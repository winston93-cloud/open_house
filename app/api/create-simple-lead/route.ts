...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🚀 Creando lead súper simple...');
    console.log('📤 Datos:', { name, phone, email });
    ...
    if (response.ok) {
      ...
      if (data._embedded && data._embedded.leads) {
        console.log(`✅ Se crearon ${data._embedded.leads.length} leads`);
        return NextResponse.json({ 
          success: true, 
          leads: data._embedded.leads.length,
