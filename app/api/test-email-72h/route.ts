...
import { getEmailTemplate72h } from '../../../lib/email-templates';
...
export async function GET(request: NextRequest) {
  const logId = `TEST_EMAIL_72H_${Date.now()}`;
  console.log(`\n🚀 [${logId}] ===== PRUEBA DE EMAIL 72H CON PLANTEL =====`);
  ...
  try {
    ...
    const emailTemplate = getEmailTemplate72h(lead.nombre, lead.plantel);

    console.log(`\n📧 [${logId}] Enviando email de prueba 72h...`);
    ...
    console.log(`✅ [${logId}] Email 72h enviado exitosamente`);

    return NextResponse.json({
      success: true,
      message: 'Email de prueba 72h enviado',
      lead: {
        nombre: lead.nombre,
        email: lead.email,
    ...
    console.log(`🏁 [${logId}] ===== FIN DE PRUEBA DE EMAIL 72H =====\n`);
  }
}
