...
export async function GET(request: NextRequest) {
  try {
    console.log('🧪 TEST LEAD SIMPLE: Solo lead sin contacto...');
    ...
    // Payload súper simple: solo lead sin contacto
    const payload = [
      {
        name: "[Open House] TEST SIMPLE",
        price: 0,
        pipeline_id: 10453492
      }
    ];
    
    console.log('📤 Payload súper simple:', JSON.stringify(payload, null, 2));
