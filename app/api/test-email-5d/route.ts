...
import { getEmailTemplate5d } from '../../../lib/email-templates';
...
export async function GET(request: NextRequest) {
  const logId = `TEST_EMAIL_5D_${Date.now()}`;
  console.log(`\n🚀 [${logId}] ===== PRUEBA DE EMAIL 5 DÍAS CON PLANTEL =====`);
  ...
  try {
    ...
    const emailTemplate = getEmailTemplate5d(lead.nombre, lead.plantel);

    console.log(`\n📧 [${logId}] Enviando email de prueba 5 días...`);
    ...
    console.log(`✅ [${logId}] Email 5 días enviado exitosamente`);

    return NextResponse.json({
      success: true,
      message: 'Email de prueba 5 días enviado',
      lead: {
        nombre: lead.nombre,
        email: lead.email,
    ...
    console.log(`🏁 [${logId}] ===== FIN DE PRUEBA DE EMAIL 5 DÍAS =====\n`);
  }
}
