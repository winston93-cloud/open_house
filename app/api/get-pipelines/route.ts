...
export async function GET() {
  try {
    ...
    // Find "Embudo de ventas" pipeline
    const embudoVentas = data._embedded.pipelines.find((p: any) => 
      p.name.toLowerCase().includes('embudo') || 
      p.name.toLowerCase().includes('ventas')
    );
    
    return NextResponse.json({
      success: true,
      allPipelines: data._embedded.pipelines,
      embudoVentas: embudoVentas || null
    });
  } catch (error) {
