...
// Template del email de recordatorio para Sesiones Informativas - COPIADO DE PRODUCCIÓN
...
// Endpoint para ENVIAR correo de recordatorio de prueba (simula 1 día antes del evento)
export async function POST(request: NextRequest) {
  try {
    ...
    // Obtener el registro de sesiones desde Supabase
    ...
    if (dbError || !sesion) {
      console.error('Error al obtener sesión:', dbError);
      return NextResponse.json(
        { error: 'Sesión no encontrada', details: dbError?.message },
        { status: 404 }
      );
    }
    ...
    const emailHtml = createSesionesReminderEmailTemplate({
      ...
      nombreCompleto: sesion.nombre_padre || sesion.nombre_madre,
      diasRestantes,
    ...
    const mailOptions = {
      ...
      to: sesion.email,
      subject: `🔔 Recordatorio - Sesión Informativa ${eventInfo.institucionNombre} (${diasRestantes} día restante)`,
      html: emailHtml
    };
    
    await transporter.sendMail(mailOptions);
    console.log(`✅ Email de recordatorio de prueba enviado a ${sesion.email}`);

    return NextResponse.json({
      success: true,
      message: 'Email de recordatorio enviado exitosamente',
      datos: {
        emailEnviadoA: sesion.email,
        nombreAspirante: sesion.nombre_aspirante,
        nivelAcademico: sesion.nivel_academico,
        fechaEvento: eventInfo.fechaEvento,
        horaEvento: eventInfo.horaEvento,
    ...
    console.error('Error al enviar email de recordatorio de prueba:', error);
    return NextResponse.json(
      { 
